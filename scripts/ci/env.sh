...
if [[ ${TRAVIS:-} ]]; then
  case ${CI_MODE} in
    js)
      ...
      ;;
    saucelabs_required)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('/home/travis/build/sinstein/angular/browser-providers.conf').sauceAliases.CI_REQUIRED.join(','))"`
      ;;
    browserstack_required)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('/home/travis/build/sinstein/angular/browser-providers.conf').browserstackAliases.CI_REQUIRED.join(','))"`
      ;;
    saucelabs_optional)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('/home/travis/build/sinstein/angular/browser-providers.conf').sauceAliases.CI_OPTIONAL.join(','))"`
      ;;
    browserstack_optional)
      setEnvVar KARMA_JS_BROWSERS `node -e "console.log(require('/home/travis/build/sinstein/angular/browser-providers.conf').browserstackAliases.CI_OPTIONAL.join(','))"`
      ;;
    aio)
...
if [[ ${TRAVIS:-} ]]; then
  ...
  export NGBUILDS_IO_KEY=${NGBUILDS_IO_KEY}
  ...
  # not using use setEnvVar so that we don't print the key
  export BROWSER_STACK_ACCESS_KEY=XVBzv9SHzyxayo1Csxbs
